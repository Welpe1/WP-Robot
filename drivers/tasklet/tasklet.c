...
static irqreturn_t key_irq_handler(int irq,void *dev)
{
    ...
    tasklet_schedule(&p_dev->tasklet); //调度tasklet,进入key_tasklet
    return IRQ_HANDLED;
...
static void key_tasklet(unsigned long data)
{
    ...
    mod_timer(&dev->key_timer,msecs_to_jiffies(10));    //开启10ms定时
}
...
static int key_gpio_init(struct key_dev *dev)
{
    ...
    /* 中断下半部初始化 */
    tasklet_init(&stKey_Dev.tasklet,key_tasklet,(unsigned long)dev);
