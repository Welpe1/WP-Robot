...
struct Current_Frame stCur_Frame={0};
...
{
    wp_set_bit((uint64_t*)&property->status,position,status);
...
{
    wp_set_bit((uint64_t*)&property->trigger,position,status);
...
void mood_get_frame_config(struct Mood_List *list)
{
    ...
    for(uint8_t i=0;i<MAX_FRAME_NUM;i++){
        ...
        if(fp!=NULL){
            ...
            fclose(fp);
            wp_set_bit((uint64_t*)&list->frame.group,i,ENABLE);
        }else{
            printf("open %s fail\r\n",frame_dir);
            wp_set_bit((uint64_t*)&list->frame.group,i,DISABLE);
        }
    }
}
